...
"""
Reverse geocoding tool for CrewAI. Converts GPS metadata to a human-friendly location name.
Returns output matching the standardized schema: {"location": <str>, "success": <bool>, "error": <str|null>}.
"""
...
class ReverseGeocodingTool(BaseTool):
    ...
    def _run(self, session_id: str, image_id: str) -> str:
        """
        Reverse geocode GPS data to location name. Returns standardized output.
        """
        try:
            store = SessionStore()
            ...
            if not gps or not gps.get('latitude') or not gps.get('longitude'):
                return json.dumps({
                    "location": "",
                    "success": False,
                    "error": "No GPS data found in image metadata."
                })
            ...
            if lat is None or lon is None:
                return json.dumps({
                    "location": "",
                    "success": False,
                    "error": "Invalid or missing GPS data."
                })
            ...
            try:
                ...
                if 'display_name' in data:
                    ...
                    return json.dumps({
                        "location": "",
                        "success": False,
                        "error": "No location found from OSM."
                    })
            except Exception as e:
                return json.dumps({
                    "location": "",
