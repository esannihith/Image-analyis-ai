...
"""
Weather data tool for CrewAI. Fetches weather conditions for an image's timestamp and GPS location.
Returns output matching the standardized schema: {"weather_report": <str>, "weather_data": <dict>, "success": <bool>, "error": <str|null>}.
"""
...
class WeatherDataTool(BaseTool):
    ...
    def _run(self, session_id: str, image_id: str) -> str:
        """
        Fetch weather data for an image. Returns standardized output.
        """
        try:
            store = SessionStore()
            ...
            if not metadata:
                return json.dumps({
                    ...
                    "weather_data": {},
                    "success": False,
                    "error": f"No metadata found for image {image_id} in session {session_id}."
                })
            ...
            if not ts or not gps or not gps.get('latitude') or not gps.get('longitude'):
                return json.dumps({
                    ...
                    "weather_data": {},
                    "success": False,
                    "error": "Missing timestamp or GPS data in image metadata."
                })
            ...
            if not api_key:
                return json.dumps({
                    ...
                    "weather_data": {},
                    "success": False,
                    "error": "No Visual Crossing API key found in environment."
                })
            try:
                ...
                if 'days' in data and data['days']:
                    ...
                    weather_report = f"Temperature: {weather_data['temperature']}°C, Cloud cover: {weather_data['cloud_cover']}%"
                    return json.dumps({
                    ...
                    return json.dumps({
                        ...
                        "weather_data": {},
                        "success": False,
                        "error": f"No weather data found for {date_str} at {lat},{lon}."
                    })
